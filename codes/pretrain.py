import os
# os.environ["CUDA_VISIBLE_DEVICES"] = "2"  # 이 줄을 주석 처리하거나 제거
import pandas as pd
import numpy as np
import argparse
...
if __name__ == '__main__':
    ...
    if args.dataset == "zinc":
        ...
        # ChEMBL_Smiles.csv 파일에서 SMILES 데이터 읽기
        data = pd.read_csv("data/ChEMBL_Smiles.csv")
        data = data['Smiles'].dropna()  # NaN 값 제거
        # 문자열이 아닌 데이터 제거
        data = data[data.apply(lambda x: isinstance(x, str))]
        # 빈 문자열 제거
