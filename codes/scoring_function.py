        ...
        try:
            ...
            if os.path.exists('./oracle'):
                try:
                    ...
                    time.sleep(0.1)  # 디렉토리가 완전히 삭제될 때까지 대기
                except:
                    print("Warning: Could not remove existing oracle directory")
            
            # JNK3 oracle 초기화 전에 임시 디렉토리 생성
            os.makedirs('./oracle', exist_ok=True)
            time.sleep(0.1)  # 디렉토리가 완전히 생성될 때까지 대기
            
            oracle = Oracle(name='JNK3', path='./oracle')
            for i in range(len(smiles)):
                if mols[i] != None:
                    try:
                        score = oracle(smiles[i])
                        if score is not None:
                            scores.append(float(score))
                        else:
                            scores.append(0.0)
                    except Exception as e:
                        print(f"Warning: Error processing molecule {i}: {str(e)}")
                        scores.append(0.0)
                else:
                    scores.append(0.0)
                    
            if not scores:  # 점수가 하나도 없는 경우
                return [0.0] * len(smiles)
            return scores
            
        except Exception as e:
            print(f"Error in JNK3 scoring: {str(e)}")
            return [0.0] * len(smiles)
